      ...
      <el-form :model="form1">
        ...
        <el-form-item label="上级菜单" :label-width="formLabelWidth">
          <el-select v-model="form1.rulesMenuId" placeholder="请选择上级菜单">
            <el-option v-for="(menu,index) in menulist" :key="index" :label="menu.MENU_NAME"
                       :value="menu.MENU_ID"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="URL地址" :label-width="formLabelWidth">
          <el-select v-model="form1.rulesUrl" placeholder="请选择URL地址">
            <el-option v-for="(url,index) in urllist" :key="index" :label="url.urlName"
                       :value="url.urlName"></el-option>
          </el-select>
        </el-form-item>
          ...
          .then(res => {
            if (res.data.status == 200) {
              ...
              for (var i = 0; i < res.data.data.length; i++) {
                var date = new Date(res.data.data[i].rulesTime)
                res.data.data[i].rulesTime = date.toLocaleString();
              }
